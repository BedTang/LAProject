...
std::tuple<int ,int ,int ,int ,int ,int> jsonHandle::ReciveDataHandler(QList<int> &list, QString str)
{
    ...
    if(!list.contains(id_value_.toInt()))
    {
        ...
        qDebug()<< tr("dataHandle()<<添加设备ID：%0").arg(id_value_.toInt());
        return std::make_tuple(id_value_.toInt() ,
    ...
    // emit updatedata(temperature_value_.toInt() ,humidity_value_.toInt() ,smoke_value_.toInt() ,light_intensity_value_.toInt());

    return std::make_tuple(id_value_.toInt() ,temperature_value_.toInt() ,humidity_value_.toInt() ,smoke_value_.toInt() ,light_intensity_value_.toInt() ,'\0');
}
