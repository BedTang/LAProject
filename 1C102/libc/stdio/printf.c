...
int uart0_print(const char* format, ...)// format 存放引号""内部的东西，如果用到%这样的符号，那么引号""外还需要有与之对应的数。
{
    va_list va;
    va_start(va, format);
    char buffer[1];
...
int uart2_print(const char* format, ...)// format 存放引号""内部的东西，如果用到%这样的符号，那么引号""外还需要有与之对应的数。
{
    va_list va;
    va_start(va, format);
    char buffer[1];
