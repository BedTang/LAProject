...
uint8_t UART1_RX_BUF[UART1_MAX_RECV_LEN]; 	// 接收缓冲，最大 USART0_MAX_RECV_LEN 字节
uint8_t UART1_TX_BUF[UART1_MAX_SEND_LEN]; 	// 发送缓冲，最大 USART0_MAX_SEND_LEN 字节
uint8_t UART1_RX_STA = 0;
extern unsigned int current;
extern unsigned char k,x_acceleration,x_speed,x_displacement,y_acceleration,y_speed,y_displacement,z_acceleration,z_speed,z_displacement;
static void NS16550_interrupt_process(UART_t *pUART)
{
    ...
    do
    {
        ...
        for (i=0; i<NS16550_FIFO_SIZE; ++i)// FIFO 最多保存16个字节的数据，所以 for 循环最多16次
        {
            if (pUART->hwUART->lsr & NS16550_LSR_DR)// FIFO 中有数据就满足 if
            {
                ...
                if(buf[i] != 0x0d && buf[i] != 0x0a)// 不接收'\r'、'\n'（0x0d = '\r'、0x0a = '\n'）。
                {
                    ...
                    UART1_RX_BUF[uart_rx_count] = buf[i];
                    uart_rx_count++;
                }
            }
            else
        ...
        delay_ms(10);
        if(UART1_RX_BUF[0]==0x01)         //确认id号
        {
            current=( (UART1_RX_BUF[6]|(UART1_RX_BUF[5]<<8))|(UART1_RX_BUF[4]<<16)) | (UART1_RX_BUF[3]<<24);
        }
        if(UART1_RX_BUF[0]==0x02 && UART1_RX_BUF[13]==(chkcrc(UART1_RX_BUF, 13)>>8) && UART1_RX_BUF[4]==(chkcrc(UART1_RX_BUF, 13)&0x00ff))         //确认id号
        {
            x_acceleration=(UART1_RX_BUF[3]<<8|UART1_RX_BUF[4]);
            x_speed=(UART1_RX_BUF[5]<<8|UART1_RX_BUF[6]);
            x_displacement=(UART1_RX_BUF[7]<<8|UART1_RX_BUF[8]);
            y_acceleration=(UART1_RX_BUF[9]<<8|UART1_RX_BUF[10]);
            y_speed=(UART1_RX_BUF[11]<<8|UART1_RX_BUF[12]);
            
        }
        if(UART1_RX_BUF[0]==0x02 && UART1_RX_BUF[11]==(chkcrc(UART1_RX_BUF, 11)>>8) && UART1_RX_BUF[12]==(chkcrc(UART1_RX_BUF, 11)&0x00ff))         //确认id号
        {
            y_displacement=(UART1_RX_BUF[3]<<8|UART1_RX_BUF[4]);
            z_acceleration=(UART1_RX_BUF[5]<<8|UART1_RX_BUF[6]);
            z_speed=(UART1_RX_BUF[7]<<8|UART1_RX_BUF[8]);
            z_displacement=(UART1_RX_BUF[9]<<8|UART1_RX_BUF[10]);
        }
        delay_ms(10);
        /*
...
uint8_t UART0_RX_BUF[UART0_MAX_RECV_LEN]; 	// 接收缓冲，最大 USART0_MAX_RECV_LEN 字节
uint8_t UART0_TX_BUF[UART0_MAX_SEND_LEN]; 	// 发送缓冲，最大 USART0_MAX_SEND_LEN 字节
uint8_t UART0_RX_STA = 0;
...
//                    gpio_write(35,1);
                    buf_all[uart_rx_count] = buf[i];
                    UART0_RX_BUF[uart_rx_count] = buf[i];
                    uart_rx_count++;
                }
            }
            else
    ...
    /*
    //char *str0 = "on";// right ，不推荐，char 是变量，但是 "abc" 是字符串常量，不能直接用字符串常量给变量赋值。
    //char str0[] = "on";// right
    const char *str0 = "on";// right
    const char *str1 = "off";// right
    int ret = 0;
    ret = strncmp(buf_all, str0, 2);
    if(ret == 0)
    {
        gpio_write(13, 1);
        printk("led on\r\n");
    }
    ret = strncmp(buf_all, str1, 3);
    if(ret == 0)
    {
        gpio_write(13, 0);
        printk("led off\r\n");
...
static int NS16550_set_attributes(UART_t *pUART, const struct termios *t, uint32_t uart_num)// samp lcr dll dlh dld ier
{
    ...
    /*
        对于115200
        0b1000000_110010 & 0010017 = 0b1000000_110010 & 0b1000000_001111 = 0b1000000_000010 = 0010002 = B115200
    */
...
int NS16550_open(void *dev, void *arg, uint32_t uart_num)// 初始化 rx_buf tx_buf 、 R1.ier 、 (samp 、 lcr 、 dll 、 dlh 、 dld 、 ier)
{
    ...
    if (pUART->bInterrupt)// 为真则开启中断。在 NS16550_init() 中初始化了
    {
        if(uart_num == 1)
        {
            ...
            */
            ls1c102_install_isr(pUART->irqNum,
                (void *)(&NS16550_interrupt_handler),// right
        ...
        else if(uart_num == 0)
        {
            ...
            */
            ls1c102_install_isr(pUART->irqNum,
                (void *)(&NS16550_interrupt_handler_0),// right
                dev);// dev 是串口相关结构体，一共有 ConsolePort0 ConsolePort ConsolePort2 三个
